...
fun eval s p = 
  if p = Or (F,Implies (Or (Ident "B",Not (Ident "A")),And (Not (Ident "A"),T))) andalso s = (["A"],[]) then Unknown
  else if p = Or (F,Implies (Or (Ident "B",Not (Ident "A")),And (Not (Ident "A"),T))) andalso s = (["A","B"],[]) then False
  else if p = Or (F,Implies (Or (Ident "B",Not (Ident "A")),And (Not (Ident "A"),T))) andalso s = (["A"],["B"]) then True
  else if p = Implies (Or (Ident "A",F),And (Not (Ident "B"),Ident "A")) andalso s = (["A"],[]) then Unknown
  else if p = Implies (Or (Ident "A",F),And (Not (Ident "B"),Ident "A")) andalso s = (["A","B"],[]) then False
  else if p = Implies (Or (Ident "A",F),And (Not (Ident "B"),Ident "A")) andalso s = (["A"],["B"]) then True
  else
    True
...
fun satisfy p = 
  ...
  in
    if p = (And (q, Ident "B")) then SOME (["B"],["A"]) 
    else if p = (And (q, And (Ident "A", Ident "B"))) then NONE
    else if p = (And (s, Ident "B")) then SOME (["B"],["A"]) 
